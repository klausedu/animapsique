...
try {
    $pdo = conectar();
    // Busca todas as publicações acadêmicas ordenadas pelo título
    $stmt = $pdo->query("SELECT id, titulo, texto FROM publicacoes_academicas ORDER BY titulo ASC");
                ...
                <?php foreach ($publicacoes as $pub): ?>
                    <div x-data="{ open: false }" class="bg-gray-50 rounded-lg shadow-sm">
                        ...
                        <div x-show="open" x-transition class="p-4 pt-0 text-gray-600 prose max-w-none">
                            <?php echo $pub['texto']; // O texto vem do editor de rich text, então não deve ser escapado ?>
                        </div>
                    </div>
                <?php endforeach; ?>
            <?php endif; ?>
